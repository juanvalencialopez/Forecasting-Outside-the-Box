function forecast(θ,X)
    ...
    for j in 1:x
        ŷⱼₜ[j] = θ[j,1]*X[1] + θ[j,2]*X[2] + X[3]*θ[j,3] + θ[j,4] 
    end
...
function heuristicAD_par(θ,T,data_yⱼₜ,X,CostMatrix)
    ...
    Threads.@threads for t in 1:T
        ...
        #println("forecasting....")
        #@show data_yⱼₜ₋₁[:,t]       
        ŷⱼₜ,δ = forecast(θ,X[t,:])   #Vector 3x1 con demanda forcasteada para el cliente j
        #@show ŷⱼₜ
