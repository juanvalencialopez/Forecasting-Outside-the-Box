...
def getRegressor(X_train,y_train,X_test,y_test):
    regressor = DecisionTreeRegressor(max_depth = 8, min_samples_leaf = 10).fit(X_train, y_train)

    y_pred = regressor.predict(X_test)
    print("score ", regressor.score(X_test, y_test))
    print("square ",mean_squared_error(y_test, y_pred))
    print(cross_val_score(regressor, X_test, y_test, cv=10,scoring="neg_mean_squared_error" ))
    return regressor
