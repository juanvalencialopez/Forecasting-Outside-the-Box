...
function bike_PlanPolicy(I,fᵢ,cᵢⱼ, x̄ᵢ,Īᵢₒ,Qᵢ,Īₙ₀,C,ξ,pᵢ,cᵢ, ratio_gᵢ,ratio_hᵢ,gᵢ,hᵢ)
    ...
    sum(fᵢ[i]*xᵢ[i] for i in 1:(I-1))+ sum(cᵢⱼ[i]*vᵢⱼ[i,i+1] - pᵢ[i]*zᵢ[i] +pos_Eᵢ[i]*cᵢ[i]  + pos_Bᵢ[i]*(cᵢ[i]/Qᵢ[i]) + pᵢ[i]*aᵢ[i] + cᵢ[i]*bᵢ[i]  for i in 1:(I-1))
    )
...
function bike_CostAssessment(xᵢ,I,fᵢ,cᵢⱼ, x̄ᵢ,Īᵢₒ,Qᵢ,Īₙ₀,C,ξ,pᵢ,cᵢ,ratio_gᵢ,ratio_hᵢ,gᵢ,hᵢ)
    ...
    + pᵢ[i]*aᵢ[i] + cᵢ[i]*bᵢ[i] for i in 1:(I-1)) 
    )
...
function heuristicAD_par(θ,T,data_xₜ,data_yₜ,I,fᵢ,cᵢⱼ, x̄ᵢ,Īᵢₒ,Qᵢ,Īₙ₀,C,pᵢ,cᵢ,L,station_ids,ratio_gᵢ,ratio_hᵢ)
    ...
    for t in 1:T #Threads.@threads for t in 1:T
        ...
        ŷⱼₜ,return_ŷⱼₜ, withd_ŷⱼₜ = py"""net_demand"""(ŷⱼₜ,station_ids)
        #@show ŷⱼₜ
        
        #------ Plan Policy, zₜ* ∈ arg min Gₚ(z,ŷₜ)

        zₜ = bike_PlanPolicy(I,fᵢ,cᵢⱼ, x̄ᵢ,Īᵢₒ,Qᵢ,Īₙ₀,C,ŷⱼₜ,pᵢ,cᵢ,ratio_gᵢ,ratio_hᵢ,return_ŷⱼₜ, withd_ŷⱼₜ )
        ...
        data_y_neta,return_data_y_neta,withd_data_y_neta = py"""net_demand"""(data_yₜ[t],station_ids) 
        costₜ = bike_CostAssessment(zₜ,I,fᵢ,cᵢⱼ, x̄ᵢ,Īᵢₒ,Qᵢ,Īₙ₀,C,data_y_neta,pᵢ,cᵢ,ratio_gᵢ,ratio_hᵢ,return_data_y_neta, withd_data_y_neta )
